 ...
 *
 * @uses       Maz_Amf_Parse_Serializer
 * @package    Maz_Amf
 * @subpackage Parse_Amf0
 * @copyright  Copyright (c) 2005-2008 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Maz_Amf_Parse_Amf0_Serializer extends Maz_Amf_Parse_Serializer
{
    /**
     ...
     * @return Maz_Amf_Parse_Amf0_Serializer
     * @throws Maz_Amf_Exception for unrecognized types or data
     */
    public function writeTypeMarker($data, $markerType = null)
    {
        if (null !== $markerType) {
            ...
            switch($markerType) {
                case Maz_Amf_Constants::AMF0_NUMBER:
                    $this->_stream->writeDouble($data);
                    break;
                case Maz_Amf_Constants::AMF0_BOOLEAN:
                    $this->_stream->writeByte($data);
                    break;
                case Maz_Amf_Constants::AMF0_STRING:
                    $this->_stream->writeUTF($data);
                    break;
                case Maz_Amf_Constants::AMF0_OBJECT:
                    $this->writeObject($data);
                    break;
                case Maz_Amf_Constants::AMF0_NULL:
                    break;
                case Maz_Amf_Constants::AMF0_MIXEDARRAY:
                    // Write length of numeric keys as zero.
                    $this->_stream->writeLong(0);
                    $this->writeObject($data);
                    break;
                case Maz_Amf_Constants::AMF0_ARRAY:
                    $this->writeArray($data);
                    break;
                case Maz_Amf_Constants::AMF0_DATE:
                    $this->writeDate($data);
                    break;
                case Maz_Amf_Constants::AMF0_LONGSTRING:
                    $this->_stream->writeLongUTF($data);
                    break;
                case Maz_Amf_Constants::AMF0_TYPEDOBJECT:
                    $this->writeTypedObject($data);
                    break;
                case Maz_Amf_Constants::AMF0_AMF3:
            ...
            switch (true) {
                case (is_int($data) || is_float($data)):
                    $markerType = Maz_Amf_Constants::AMF0_NUMBER;
                    break;
                case (is_bool($data)):
                    $markerType = Maz_Amf_Constants::AMF0_BOOLEAN;
                    break;
                case (is_string($data) && (strlen($data) > 65536)):
                    $markerType = Maz_Amf_Constants::AMF0_LONGSTRING;
                    break;
                case (is_string($data)):
                    $markerType = Maz_Amf_Constants::AMF0_STRING;
                    break;
                case (is_object($data)):
                    if (($data instanceof DateTime) || ($data instanceof Zend_Date)) {
                        $markerType = Maz_Amf_Constants::AMF0_DATE;
                    } else {

                        if($className = $this->getClassName($data)){
                            //Object is a Typed object set classname
                            $markerType = Maz_Amf_Constants::AMF0_TYPEDOBJECT;
                            $this->_className = $className;
                        } else {
                            // Object is a generic classname
                            $markerType = Maz_Amf_Constants::AMF0_OBJECT;
                        }
                        break;
                    }
                    break;
                case (null === $data):
                    $markerType = Maz_Amf_Constants::AMF0_NULL;
                    break;
                case (is_array($data)):
                    // check if it is a mixed typed array
                    foreach (array_keys($data) as $key) {
                        if (!is_numeric($key)) {
                            $markerType = Maz_Amf_Constants::AMF0_MIXEDARRAY;
                            break;
                        }
                    }
                    // Dealing with a standard numeric array
                    if(!$markerType){
                        $markerType = Maz_Amf_Constants::AMF0_ARRAY;
                        break;
                    }
                    break;
                default:
    ...
    public function writeObject($object)
    {
        ...
        $this->_stream->writeByte(Maz_Amf_Constants::AMF0_OBJECTTERM);
        return $this;
    }
    ...
    public function writeDate($data)
    {
        if ($data instanceof DateTime) {
            ...
            require_once 'Maz/Amf/Exception.php';
            throw new Maz_Amf_Exception('Invalid date specified; must be a DateTime or Zend_Date object');
        }
        $dateString *= 1000;
    ...
    {
        require_once 'Maz/Amf/Parse/Amf3/Serializer.php';
        $serializer = new Maz_Amf_Parse_Amf3_Serializer($this->_stream);
