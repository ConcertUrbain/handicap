...
/** Maz_Amf_Constants */
require_once 'Maz/Amf/Constants.php';

/** Maz_Amf_Value_MessageBody */
require_once 'Maz/Amf/Value/MessageBody.php';

/** Maz_Amf_Value_Messaging_CommandMessage */
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Maz_Amf_Server implements Zend_Server_Interface
{
    /**
    ...
    /**
     * Request processed
     * @var null|Maz_Amf_Request
     */
    protected $_request = null;

    /**
     * Class to use for responses
     * @var null|Maz_Amf_Response
     */
    protected $_response;
     ...
     * @throws Maz_Amf_Server_Exception
     */
        ...
        if (!isset($this->_table[$method])) {
            ...
            if ($source) {
                ...
                try {
                    ...
                    require_once 'Maz/Amf/Server/Exception.php';
                    throw new Maz_Amf_Server_Exception('Class "' . $className . '" does not exist');
                }
                // Add the new loaded class to the server.
                $this->setClass($className);
            } else {
                require_once 'Maz/Amf/Server/Exception.php';
                throw new Maz_Amf_Server_Exception('Method "' . $method . '" does not exist');
            }
        }
        ...
        if ($info instanceof Zend_Server_Reflection_Function) {
            ...
            if ('static' == $info->isStatic()) {
                ...
                try {
                    ...
                    require_once 'Maz/Amf/Server/Exception.php';
                    throw new Maz_Amf_Server_Exception('Error instantiating class ' . $class . ' to invoke method ' . $info->getName(), 621);
                }
                $return = $info->invokeArgs($object, $params);
            }
        } else {
            require_once 'Maz/Amf/Server/Exception.php';
            throw new Maz_Amf_Server_Exception('Method missing implementation ' . get_class($info));
        }
     ...
     *
     * @see    Maz_Amf_Value_Messaging_CommandMessage
     * @param  Maz_Amf_Value_Messaging_CommandMessage $message
     * @return Maz_Amf_Value_Messaging_AcknowledgeMessage
     */
    protected function _loadCommandMessage(Maz_Amf_Value_Messaging_CommandMessage $message)
    {
        switch($message->operation) {
            case Maz_Amf_Value_Messaging_CommandMessage::CLIENT_PING_OPERATION :
                require_once 'Maz/Amf/Value/Messaging/AcknowledgeMessage.php';
                $return = new Maz_Amf_Value_Messaging_AcknowledgeMessage($message);
                break;
            default :
                require_once 'Maz/Amf/Server/Exception.php';
                throw new Maz_Amf_Server_Exception('CommandMessage::' . $message->operation . ' not implemented');
                break;
        }
        return $return;
    }
     ...
     * @param  Maz_Amf_Request $request
     * @return Maz_Amf_Response
     * @throws Maz_Amf_server_Exception|Exception
     */
    protected function _handle(Maz_Amf_Request $request)
    {
        ...
        {
            try {
                if ($objectEncoding == Maz_Amf_Constants::AMF0_OBJECT_ENCODING) {
                    ...
                    if ($message instanceof Maz_Amf_Value_Messaging_CommandMessage) {
                        // async call with command message
                        $return = $this->_loadCommandMessage($message);
                    } elseif ($message instanceof Maz_Amf_Value_Messaging_RemotingMessage) {
                        require_once 'Maz/Amf/Value/Messaging/AcknowledgeMessage.php';
                        $return = new Maz_Amf_Value_Messaging_AcknowledgeMessage($message);
                ...
                switch ($objectEncoding) {
                    case Maz_Amf_Constants::AMF0_OBJECT_ENCODING :
                        $return = array(
                        ...
                        break;
                    case Maz_Amf_Constants::AMF3_OBJECT_ENCODING :
                        require_once 'Maz/Amf/Value/Messaging/ErrorMessage.php';
                        $return = new Maz_Amf_Value_Messaging_ErrorMessage($message);
            ...
            $newBody     = new Maz_Amf_Value_MessageBody($responseURI, null, $return);
     ...
     *
     * @param  null|Maz_Amf_Request $request Optional
     * @return Maz_Amf_Response
     */
    public function handle($request = null)
    {
        // Check if request was passed otherwise get it from the server
        if (is_null($request) || !$request instanceof Maz_Amf_Request) {
        ...
        try {
            ...
            // Serialize to Maz_Amf_response for output stream
            $this->_handle($request);
            $response = $this->getResponse();
        } catch (Exception $e) {
            // Handle any errors in the serialization and service  calls.
            require_once 'Maz/Amf/Server/Exception.php';
            throw new Maz_Amf_Server_Exception('Handle error: ' . $e->getMessage() . ' ' . $e->getLine());
        }
     ...
     * Set request object
     *
     * @param  string|Maz_Amf_Request $request
     * @return Maz_Amf_Server
     */
    public function setRequest($request)
    {
        if (is_string($request) && class_exists($request)) {
            $request = new $request();
            if (!$request instanceof Maz_Amf_Request) {
                require_once 'Maz/Amf/Server/Exception.php';
                throw new Maz_Amf_Server_Exception('Invalid request class');
            }
        } elseif (!$request instanceof Maz_Amf_Request) {
            require_once 'Maz/Amf/Server/Exception.php';
            throw new Maz_Amf_Server_Exception('Invalid request object');
        }
     ...
     *
     * @return null|Maz_Amf_Request
     */
    public function getRequest()
    {
        if (null === $this->_request) {
            require_once 'Maz/Amf/Request/Http.php';
            $this->setRequest(new Maz_Amf_Request_Http());
        }
    ...
    /**
     * Public access method to private Maz_Amf_Server_Response refrence
     *
     * @param  string|Maz_Amf_Server_Response $response
     * @return Maz_Amf_Server
     */
    public function setResponse($response)
    {
        if (is_string($response) && class_exists($response)) {
            ...
            if (!$response instanceof Maz_Amf_Response) {
                require_once 'Maz/Amf/Server/Exception.php';
                throw new Maz_Amf_Server_Exception('Invalid response class');
            }
        } elseif (!$response instanceof Maz_Amf_Response) {
            require_once 'Maz/Amf/Server/Exception.php';
            throw new Maz_Amf_Server_Exception('Invalid response object');
        }
    ...
    /**
     * get a refrence to the Maz_Amf_response instance
     *
     * @return Maz_Amf_Server_Response
     */
    public function getResponse()
    {
        if (null === ($response = $this->_response)) {
            require_once 'Maz/Amf/Response/Http.php';
            $this->setResponse(new Maz_Amf_Response_Http());
        }
     ...
     * @return Maz_Amf_Server
     * @throws Maz_Amf_Server_Exception on invalid input
     */
    public function setClass($class, $namespace = '', $argv = null)
    {
        if (is_string($class) && !class_exists($class)){
            require_once 'Maz/Amf/Server/Exception.php';
            throw new Maz_Amf_Server_Exception('Invalid method or class');
        } elseif (!is_string($class) && !is_object($class)) {
            require_once 'Maz/Amf/Server/Exception.php';
            throw new Maz_Amf_Server_Exception('Invalid method or class; must be a classname or object');
        }
    ...
    public function addFunction($function, $namespace = '')
    {
        if (!is_string($function) && !is_array($function)) {
            require_once 'Maz/Amf/Server/Exception.php';
            throw new Maz_Amf_Server_Exception('Unable to attach function');
        }
    ...
    protected function _buildDispatchTable()
    {
        ...
        foreach ($this->_methods as $key => $dispatchable) {
            if ($dispatchable instanceof Zend_Server_Reflection_Function_Abstract) {
                ...
                if (isset($table[$name])) {
                    require_once 'Maz/Amf/Server/Exception.php';
                    throw new Maz_Amf_Server_Exception('Duplicate method registered: ' . $name);
                }
